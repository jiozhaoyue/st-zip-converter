...
## Session 30: 六实例数据同源 + 插件功能自动化验证（备份完成；产包被产品缺陷阻断）

### Summary

用户要求「全部自动化验证插件功能 + 把 Real Luker 的配置同步到所有实例（没有的部分就不要）」。
规划期先把范围钉死（六实例、含角色卡与聊天、覆盖同名不删独有、排除 `backups/`、
PT 全自动 + TT 人工、授权启动实例），**阶段 1 备份**干净完成，
但**阶段 2 产包被 `src/core/zip-io.js` 的一串缺陷阻断**——三个缺陷互为表里，
且**产品自身的 Node 路径也挂**。修掉 A–D 后吞吐从 0 字节提升到 300 MB / 2000 条，
**但全量仍会停滞（缺陷 E 成因未定位）**。按用户裁决改道：
保留 A–D、转做不依赖产包的部分 —— E2E 基础设施 + 加载冒烟 + OQ-1 探路全部完成。

### Main Changes

- **阶段 1 备份（✅ AC-1）**：Real Luker 原生数据包
  `backup-real-luker-20260926-010422.zip` = 1602.7 MB 压缩 / **3840.6 MB 未压缩** /
  8683 条目 / 247.1 s，附 `bytes/entryCount/sha256` 记录。
  探针先证通道：`chunked` + **无 `content-length`** ⇒ 否掉「Playwright 下载」与「整包进内存」，
  改「浏览器只取会话 + Node 侧 `https` 流式直写磁盘」。
  体检出未压缩 3840.6 MB 与源目录量级一致、一级构成逐项对得上（**不靠「体积看着差不多」**）。
  实测速率塌方：进入 `backups/` 后 7 MB/s → 0.9 MB/s（对已 deflate 的历史包再压一遍，是 K-1 的真实形态）。
- **四处产品缺陷（A–D，已修并保留，零回退）**：
  A `openStream` 在已锁流上 `cancel` ⇒ `ERR_INVALID_STATE` 逃逸成**未处理拒绝杀死 Node 进程**且掩盖主错误；
  B `bufferedWrite: true` 硬编码 ⇒ 落点恒 0 字节、`close()` 永不返回（改 `false` 后 82.4 s / 614 MB / RSS 330 MB）；
  C 背压窗口放在 `firstEntryDone` 之后 ⇒ 紧凑投递下**全部走首条目分支、无一经过背压**（无界并发）；
  D `track()` 把**排队者**也算进 `inflight` ⇒ 排队数一超 CONCURRENCY 就**永久自锁**
  （纯形态复刻实测 100 条只完成 6 条）⇒ 改 `inflight`/`pending` 分离 + 显式首条目闸门。
- **E2E 基础设施（✅ AC-5 / AC-6 / AC-8）**：`e2e/` 入库，`npm run e2e` = **断言 53 项全过、exit 0**。
  端口守卫 17 项负例全过，含「Real 端口在**启动浏览器之前**就抛错」的**集成证明**
  （证明守卫真的接在自动化路径上，不是「写了个函数没人用」）。
  归因纪律落地：读数分「整页背景（不判定）」与「本插件归因（判定只看它）」两栏。
- **OQ-1 推翻设计假设**：ST 1.19.0 **没有整包导入、也没有整合的导入 UI**（`users-private.js` 只有 `/backup`）。
  可达的只有逐类目端点，且有四条硬约束（顺序强制、`settings/save` 整份替换与 U-3 冲突、
  预设主题无通道、约 1500 次调用）⇒ **「源覆盖率 100%」在 ST 目标上不可达**，`design.md` §5.2 已更正。
- **规范落库**：新建 `backend/node-zip-writer-pitfalls.md` 与 `guides/instance-e2e-and-data-sync.md`
  （均自包含），更新三处索引。
...
| Hash | Message |
|------|---------|
| `1f16aa2` | fix(zip-io): 修 4 处缺陷（解压流 cancel、bufferedWrite、无界并发、背压自锁） |
| `ae8c8d7` | test(e2e): 实例同源同步工具 + 端口守卫；Trellis 任务 09-26 规划与取证 |
| `16d835c` | test(e2e): E2E 基础设施 + 加载冒烟（53 项断言全过）+ 实例启停器 + 规范落库 |

### Testing

- [OK] `npm test` **46 文件 / 429 passed / 2 skipped，exit 0** —— 与基线逐项一致、零回退
- [OK] 五条静态守卫 `exit=0`；`npm run build` `✓ built in 1.26s`；`npm run e2e` **53/53、exit 0**
- [OK] 备份完整性：未压缩 3840.6 MB 与源目录量级一致，一级构成逐项对上
- [注意] 登记一条**既有抖动**（R-10）：`test/real-samples.test.js` 5 s 超时，全量 5 次抖 2 次、
  单跑 3/3 全过。根因是该用例读 `out/` 下 **560 MB** 真实产物，而 `openReader` 在 Node 上整包进内存。
  **与本次改动无关**（该文件只用 `openReader`+`skip`，`createWriter`/`openStream` 出现 0 次）。已另开后台任务建议
- [注意] 修正**三处我自己的错误**：① 实例登记表把 ST 写成 `https://`（实为明文 HTTP，被就绪探针以
  `HTTP EPROTO` 抓出）；② 先前用 `grep "LISTENING.*:port"` 判端口**顺序写反**，误得「四端口全未监听」
  （实际 8003/8004 本就在跑，端口归属已用探针决定性确认）；③ 冒烟三条断言写错（抽屉态无 `.app-header`、
  未开弹层时注入按钮本应为 0、菜单项 **t≈7 s** 才注入）⇒ 回源码核实后改成契约相对断言 + 有界等待

### Status

[OK] **阶段 1 完成、阶段 2/3 挂起**（残留 R-1…R-10 已登记；最关键的是**产包与同步未执行**，
以及 ST 目标的同步通道需按 OQ-1 重新设计）

### Next Steps

- **缺陷 E** 待专项定位：`convert()` 在真源包全量上仍停滞（首次落盘 5.0 s、300 MB / 2000 条后停滞；
  纯全量投递 5.8 MB 后停滞）。已证明**可用**的写法是顺序或批次 8 条（614 MB / 72.5–82.4 s）
- R-2：ST 目标同步通道按逐类目端点重设计（含 `settings` 先读回再合并、预设主题无通道的降级）
...
## Session 33: PT 同步打通（推翻两条旧结论）+ N-1 归一修复与单测 + 一处方法论发现

### 用户本轮裁决（三条）

| # | 决策点 | 选定 |
| --- | --- | --- |
| U-13 | PT 怎么处置（原以为是「起 remote-server」） | **起 remote-server 并推进** —— 但该前提经实地证伪，实际改走了正确路径 |
| U-14 | 疑似缺陷 N-1（`native` 未走布局码归一） | **本轮修 + 补单测** |
| U-15 | 实例收尾（AC-11） | **暂时保持运行** |

### 一、推翻两条旧结论（都是我自己先前写进 research 的）

1. **「PT web 模式没有导入后端」——错。**
   PT 的 `/api/*` 是 PT **纯前端**自己注册的 legacy 兼容路由
   （`apps/web/src/features/import-export/legacy/register-routes.ts`），
   由 `legacy-hook/bootstrap.ts` 的 `installCompatibilityFetch(router)`
   **补丁页面内的 `window.fetch`** 生效。页面内实测：
   `POST /api/backups/archive/inspect` → **200**（返回真实的 659 records / 329 blobs）；
   `POST /api/backups/tauritavern/import/preview` → **400**（参数缺失，**不是 404**）。
   **早先的 404 是探查方法的伪影** —— 探查绕过了页面内被补丁的 fetch。
2. **「起 remote-server 能帮上忙」——错且方向相反。**
   `apps/remote-server` 是 **LLM 请求代理**（README 明写只有 `GET /v1/health` 与 `POST /v1/proxy`），
   与备份/导入毫无关系。用户选「起 remote-server 并推进」是基于这个错误前提，已回报。

> 教训：**「拿不到」的第一嫌疑应该是「我的探查方式不对」，而不是「能力缺失」。**
> 本次 404 与 400 的差别就是全部答案，而我把 404 直接当成了结论。

### 二、PT 同步完成（新增 `import-pt.cjs`）

实际通道 = 宿主数据管理面板（独立 `<dialog>`，由「打开数据管理」按钮打开；
**不是**点设置项抽屉展开 —— 先前 `pt-automation.cjs --import-pack` 正栽在这里）。

排掉的 **4 个坑**（每个都实际踩过）：

1. 只投文件**不会开始** —— 必须点「执行 TauriTavern 导入」。
2. 该按钮在**预览完成前是 disabled**（统一投文件只触发异步预览）⇒ 投完就点 = 静默不走。
3. **确认是两层、且用同一个选择器** `[data-action="confirm"]`（①「继续」→ ②「确定」），
   且**两层之间隔着逐文件 CRC/SHA-256 校验**（7209 文件 / 1.16 GB 实测数分钟）；
   先前只轮询 9 秒 ⇒ **等不到第二层** ⇒ 导入根本没开始，外层却在傻等。
   改为**时长预算**模型（校验期持续等待 + 每 30s 报进度）。
4. 完成信号是**页面 reload** 而不是浮层文本 —— 源码 `notify(...)` 后立刻
   `setTimeout(location.reload, 500)`，浮层只存在约 500 ms，3s 轮询必然错过
   （症状：计数已 +15759 却被判「超时」）。改用 `framenavigated` 事件。

**读数**（pack-tt 510.8 MB）：耗时 **337.3s**、两层确认全过、records **976→11740（+10764）**、
blobs **334→5326（+4992）**、**只增不减**（U-3）；**体积精确吻合**（PT 侧合计 ≈1161.6 MB
= 源包未压缩 1.16 GB）；聊天角色目录 **23=23**、「孤独摇滚」系列 **165=165**。
另澄清一处口径：源包 `chats/` 的 1029 条 = 223 真聊天 `.jsonl` + 786 `.luker-state.*` + 20 `runs/**`
（同缺陷 F-4 的形态：**文件条目数 ≠ 内容数**）。

配套：`lib/pt-count.cjs`（计数装置唯一实现，两处共用防漂移）、`--report` 只读核对装置。

### 三、N-1 修复（用户批准）

- 新增纯函数 `resolveTargetLayout(rawValue, platform, fallback)`（`src/ui/host-bridge.js`）
  作为**唯一归一实现**；`index.js` 的 **5 处**取目标全部改走它。
- 单测 `test/target-resolve.test.js` **11 项**，含 `native + luker ⇒ l` 的**核心负例**；
  **已实测证明判别力**：临时去掉归一后 **6 项转红**、恢复后 11/11。
- E2E 侧把原「差异登记」断言改成了**一致性断言**（`native` 读数须等于该宿主的显式布局码读数）。

### 四、方法论发现：E2E 验证的是**实例里装的插件**，不是工作区代码

N-1 修复后复跑 E2E，两条 `native` 一致性断言**如期失败** —— 查实为
**实例里的插件停在 `1640118`**（`git merge-base --is-ancestor` 确认是 HEAD 的**祖先**），
**不含我的修复**。⇒ **此前 E2E 的全部读数（53/53、175/175）验证的都是旧代码。**

这反过来**证明该断言有效**（它能捕获「实例未更新」这件事）。已登记为 **R-20**，
并形成纪律：用 E2E 验证新代码前，先把实例插件更新到目标版本（走 `git pull`，
L0-1 允许的实例更新通道；`origin` 已确认指向本仓、非上游），且**只更新 Dev 实例**。

### Git Commits

- `b7f3fda` fix(sync): ST 目标通道缺陷修复（拆壳/口径/缓存/链接守卫）+ settings 交集合并
- `af83b84`/`cbf4f1e` test(e2e): 功能矩阵 M-1…M-9 落地（122 项断言 · 连续两轮全绿）
- `de66b65` fix(host): N-1 —— 目标选择器的 native 未走布局码归一（唯一实现 + 单测）
- `dbee663` feat(sync): PT（PureTavern）数据同步打通 —— 两条先前结论被实地证伪

### Testing

- [OK] `npm test` **48 passed / 1 skipped（49 文件）、460 passed / 2 skipped** ——
  基线 47/449 之上**新增正好 1 文件 11 项**（N-1 单测），**零回退**
- [OK] 五条静态守卫全 `exit=0`；`npm run build` ✓ 818ms
- [OK] `npm run e2e` **175 项 / 通过 173 / 失败 2** —— 两条失败**已查实是「实例代码旧」**（R-20），
      非断言错误；更新实例后再复验
- [注意] PT 侧 `--report` 与导入读数均只增不减 ✓

### Status

[OK] **PT 同步完成**（此前是最后一块未同步的实例）；**N-1 已修并单测守护**
[待办] **R-20**：把 Dev 实例插件更新到新代码后复跑 E2E，方可宣称「矩阵在**新代码**上全绿」
[待办] AC-11 收尾（用户选「暂时保持运行」，未执行）
[待办] PT 残留 PT-R1…PT-R4（卡片级未逐张核对 / best-effort 存储风险 / `runs/**` 去向 / 坏分支留档）
[待办] `task.py archive` 未执行（任务未完成）


### 五、R-20 的处置与最终复验（本轮收尾）

按 L0-1 允许的通道（`git pull`；两目录均干净、`origin` 已确认指向本仓、非上游）把**两个 Dev 实例**的
插件从 `1640118` 更新到 `b16b5ba`（**只更新 Dev**；Real 实例未动）。
**更新后全量 `npm run e2e` = 断言 175 项 / 通过 175 / 失败 0（exit 0）** ——
**矩阵首次在「与工作区同版本」的代码上全绿**，N-1 的两条一致性断言随之通过。

⇒ 结论：**R-20 不只是「一个待办」，它证明了两件事** ——
① 那两条断言**真的能捕获**「实例代码过期」这类问题（修复前它们如期报红）；
② 在玩家真实环境里跑 E2E 时，**「实例版本」是一个必须显式核对的前置条件**，否则读数不可解释。
