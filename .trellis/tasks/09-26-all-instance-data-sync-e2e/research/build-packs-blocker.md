...
# 缺陷 E 的修复与验证（2026-09-26 · 已实施）

> **用户授权**：2026-09-26 交互问答中，用户在确认「是插件的源码（`src/core/`，非酒馆源码）」后，
> 选定「按推荐做」⇒ **批准修改本插件源码**。酒馆源码（`Instance/**`）全程未动。

## 采用的修法：投递侧背压（方向 ①）

**为什么选它**：诊断已把可行方向收敛到一条 —— 全量投递时「在飞」可达 **916 条**（窗口仅 8），
而**已有的对照实验**（同一源包、同一 io 接缝）早就表明：分批投递只要 **72.5 s**，
比全量投递的 **182 s** 还快，且 RSS 仅 330 MB。**内存与吞吐同向改善**，不是权衡。

改动共两处，均很小：

| 文件 | 改动 |
| --- | --- |
| `src/core/zip-io.js` | `waitForRoom(maxPending = CONCURRENCY)` —— 由「等全部落盘」改为「等积压降到上限之下」。等待时对 pending 做 `catch(() => {})`：条目写入失败本就在 `close()` 的 `allSettled` 汇合，这里不吞掉只会**新增**一个抛出点、改变既有错误传播形态。 |
| `src/core/transform.js` | 新增常量 `WRITE_BATCH = 16`；主循环每处理 16 个源条目 `await writer.waitForRoom()`。`dryRun` 走 `NullZipWriter`（其 `waitForRoom()` 本就是空实现），无需分支。 |

**其余三个方向经诊断被排除**，不采：
- ② 收敛 worker 数：**降 worker 反而更早 OOM**（伪造 HW=4/2 时 T+47s/44s 就崩）；
- ③ heap snapshot：`--heapsnapshot-near-heap-limit=1` 在本机**不可用** —— 写快照本身的内存压力
  让进程直接死亡，只留 **0 字节**文件（1536 MB 与默认堆两种配置都试过）；
- 附带排除：与 worker 通信无关（`--no-workers` 下 heap 仍冲到 6195 MB）、
  与数据 buffer 无关（峰值时 `external` 仅 76 MB）。

## 修复前后实测对照（同一源包 1602.7 MB / 8683 条）
...
| 单目标耗时 | **182.0 s**（且**必须** `--max-old-space-size=8192`） | **54.8 s**（**默认堆**） |
...
⇒ 总耗时约 **3.1 倍**加速，产出体积逐项一致（619.3 / 619.3 / 619.6 MB），
`verify-packs.cjs` 的 A1–A4 断言**全部通过**。

## 质量门

- `npm test`：**45 passed / 1 skipped（46 文件）、429 passed / 2 skipped** —— 与基线**逐项一致、零回退**。
- 三份产包复校验：A2 `backups/`=0、A3 `characters/`=430 与 `chats/`=1029、A4 布局前缀 —— 全过。

## 仍未做到的事（**如实登记**）

**没有为这个修复新增单测**，且这是有意的：该缺陷**无法被快速单测守护** ——
小规模（84 MB / 60 条）下两种投递方式都正常，**唯一触发维度是总规模（GB 级）**，
注定进不了 `npm test`。可执行守护因此放在
`node scripts/instance-sync/selftest-node-zip-io.cjs --large <真源包.zip>`
与本节上表的实测读数里。
