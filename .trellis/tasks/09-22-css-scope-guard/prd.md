...
- [ ] 当前 `style.css` 通过独立 npm 命令和 `npm test` 中的作用域测试。
- [ ] 测试证实顶层、媒体查询、支持查询及逗号列表里的裸选择器均被拒绝，并显示对应位置；两个合法前缀及独立页面专用骨架规则通过。
- [ ] 测试证实 `@keyframes` 百分比帧不会被误判，裸 `body`、`:root`、`*`、`.menu_button` 不能绕过校验。
- [ ] 响应式规则修复后，独立页面和宿主插件的对应选择器仍分别有匹配规则；`npm test` 和 `npm run build` 全绿。
- [ ] `git diff` 可清晰审查仅作用域及守卫相关的变更；不改变页面结构或转换逻辑。
