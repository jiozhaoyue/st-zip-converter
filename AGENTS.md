...
**规则**：
- 本仓群的子代理**一律使用能力较低的小模型**，禁止用旗舰模型做子代理。当前指定：`glm-5.3-flash`（2026-09-23 起）。**禁止使用 Kimi K3 做子代理**。
- **派发子代理前必须核验实际生效模型**：派发后立即检查端点/回执确认请求确实落在指定模型上；若实际路由到了别的模型（尤其旗舰模型），**立即停止该子代理**并换模型重派。实测事故：显式指定 `DeepSeek-V4-Flash[free]` 后上游日志仍显示路由到 `gpt-5.6-sol`（2026-09-23）。
- **未明确指定子代理模型时，必须停下询问**：列出当前平台可用模型清单，经用户选定后再派发，严禁默认继承主线程旗舰模型。
