...
> ⚠ **历史存档（2026-09-07）——不是现行规范。**
> 本文件记录当时的「两块式」设计。此后已发生（至少）两次结构演进：
> 「两块式」→ 分区式（`wb-block` + A–K 分区，见 `09-25-ui-slim-native`）、
> 以及单一模板源改造（`index.html` 现为空骨架）。
> 文中提到的 `#incremental-mode-check` 已于 `09-25-transfer-pack-optimize` 移除
> （取证确认其值从不被读取）。
